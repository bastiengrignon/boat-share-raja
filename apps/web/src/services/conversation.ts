...
export const conversationService = {
  ...
  archiveConversation: async ({ conversationId, userId }: QueryConversationId & QueryUserId) =>
    (await boatSharingApi.post(`${conversationRoute}/${conversationId}/archive`, { userId })).data,
  getArchivedConversations: async ({ userId }: QueryUserId) =>
    (await boatSharingApi.get(`${conversationRoute}/${userId}/archive`)).data,
};
